...
export default function EditProduktPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/produkte/${params.id}`);
        ...
        if (data.product) {
          reset({
            ...
            vatRate: Number(data.product.vatRate),
            unit: data.product.unit,
          });
        }
      } catch {
        toast({ title: "Fehler", description: "Produkt konnte nicht geladen werden.", variant: "destructive" });
      } finally {
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        toast({ title: "Gespeichert", description: "Produkt wurde erfolgreich aktualisiert." });
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="vatRate" type="number" step="0.01" min="0" max="100" {...register("vatRate")} />
